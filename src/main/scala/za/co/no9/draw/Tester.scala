	...
	def main(args: Array[String]) {
		Image(
			List(
				...
				new BoxShape(layoutPoint = (previous: LayedOutShape) => LayoutPoint(West(), At(previous.last(0).get.grips.east, Point(0, 0))), width = 100, height = 20, lineStyle = Some(LineStyle(width = 2.0f, colour = RED)), text = Some(Text("Cheerio", TextStyle(name = "Arial", size = 10, style = Plain(), colour = GREEN)))),
